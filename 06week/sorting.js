...
// press alt + z to insert a line break to include the entire array in view

// Given 1000 digits of PI as strings, return an array of the digits as numbers
const stringsToNumbs = (numbers) => {
    return numbers.map(Number);
    // arr.map(Numbers) turns an array of numers in a type string into an array of numbers as type numbers 
}
console.log(stringsToNumbs(strNums));
// make a variable to continue to use the array as type numbers
let nums = stringsToNumbs(strNums)

// With the same numbers, find the sum of the even values
// the parameter takes in an array
const sumEvens = (evens) => {
    newArr = [];
    // create a new array to push to
    for (let i = 0; i < evens.length; i++) {
        // the for loop will iterate through the array of numbers
        if (evens[i] % 2 == 0) {
            // the if statement checks to see if each item in the array is evenly divisable by 2, thus finding all of the even numbers
            newArr.push(evens[i]);
            // now push all of the even numbers to the new array
        }
    }
...
const atxIdx = (atx) => {
    for (let i = 0; i < atx.length; i++) {
        if (atx[i] + i == 512) {
            // check to see if the index and its value is equal to 512
        return i
        }
    }
}
console.log(`index: ${atxIdx(nums)}, value: ${nums[atxIdx(nums)]}`);
// the function returns i so when the function is called it just returns the index and when the function is called inside square brackets
// after an array it would be like saying array[i] and provide the value
...
let states = [];
// decalre a global array to push to
const weatherStates = (arr) => {
    arr.map((currentValue, index, array) => {
        // we only actually use the currentValue but it is good practice to see how map would be written out
        if (!states.includes(currentValue.weather_state_name)) {
            // if states array does not(!) include(.includes) curreentValue(weather).weather_state_name
        states.push(currentValue.weather_state_name)
        // push to the states array
        }
    })
    return states;
    // return the states array. make sure it is within the function but not within the map function(for loop)
}
...
const idealTemp = (arr) => {
    arr.filter((currentValue, index, arry) => {
